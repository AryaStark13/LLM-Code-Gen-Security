...
Modified to handle multiple eval datasets independently.
"""
...
#     Returns:
#         (train_dataset, eval_datasets_dict)
#         eval_datasets_dict is a dict where keys are dataset names and values are datasets
#     """
#     train_datasets = []
#     eval_datasets_dict = {}
...
#         # Create short name for eval dataset key
#         dataset_short_name = name.split('/')[-1]
...
#         # Load and concatenate eval splits for this dataset
#         if eval_splits:
...
#                 dataset_eval_list.append(eval_ds)
            
#             # Concatenate eval splits for this dataset
#             if len(dataset_eval_list) > 1:
#                 eval_datasets_dict[dataset_short_name] = concatenate_datasets(dataset_eval_list)
#             else:
#                 eval_datasets_dict[dataset_short_name] = dataset_eval_list[0]
            
#             print(f"  - Added '{dataset_short_name}' to eval with {len(eval_datasets_dict[dataset_short_name])} examples")
...
#     total_eval = sum(len(ds) for ds in eval_datasets_dict.values())
#     print(f"\nTotal: {len(train_dataset)} train examples")
#     print(f"Eval datasets: {list(eval_datasets_dict.keys())}")
...
def load_datasets(dataset_configs: List[Dict]) -> tuple:
    ...
    Returns:
        (train_dataset, eval_datasets_dict)
        eval_datasets_dict is a dict where keys are split names and values are datasets
    """
    train_datasets = []
    eval_datasets_dict = {}
    ...
    for config in dataset_configs:
        ...
        if eval_splits:
            ...
            for eval_split in eval_splits:
                ...
                # Create a unique key for this eval split
                # Format: {dataset_short_name}_{split_name}
                eval_key = f"{dataset_short_name}_{eval_split}"
                eval_datasets_dict[eval_key] = eval_ds
                
                print(f"  - Added '{eval_key}' to eval with {len(eval_ds)} examples")
...
def main():
    ...
    # Load datasets - now returns dict of eval datasets
    train_dataset, eval_datasets_dict = load_datasets(config['datasets'])
    ...
    # Create trainer with dict of eval datasets for independent evaluation
    trainer = CustomSFTTrainer(
        model=model,
        args=sft_config,
        train_dataset=train_dataset,
        eval_dataset=eval_datasets_dict,  # Pass dict instead of single dataset
