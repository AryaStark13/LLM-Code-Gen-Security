...
def main():
    ...
    print(f"Total parameters in the model: {sum(p.numel() for p in base_model.parameters()):,}")
    ...
    if args.run_eval:
        ...
        run_full_evaluation(base_model, tokenizer, stopping_criteria, eval_dataset, args.eval_output)
    else:
        # Example inference
        example_prompt = eval_dataset[0]["prompt"][0]["content"]
        generated_output = generate_output(example_prompt, base_model, tokenizer, stopping_criteria)
