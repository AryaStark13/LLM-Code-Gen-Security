...
def load_and_merge_grpo_lora_adapter(sft_merged_model_path, grpo_lora_adapter_path, max_seq_length=2048, load_in_4bit=False):
    ...
    print(f"Loading SFT merged model from: {sft_merged_model_path}")
    ...
    print(f"Loading GRPO LoRA adapter from: {grpo_lora_adapter_path}")
...
def main():
    ...
    if args.run_eval:
        ...
        table = [
            ...
            [grpo_generated_output, ground_truth]
        ]
        print(tabulate(table, headers="firstrow", tablefmt="grid"))

        # save output to a text file
        with open("grpo_lora_model_example_output.txt", "w") as f:
