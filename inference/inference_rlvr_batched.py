...
def merge_rlvr_lora_adapter(rlvr_lora_model):
    merged_model = rlvr_lora_model.merge_and_unload()
    return merged_model
...
def main():
    ...
    rlvr_merged_model = merge_rlvr_lora_adapter(rlvr_lora_model)
    ...
    print("Model is ready for inference with SFT + RLVR LoRA adapters.")
    print(f"Total parameters in the model: {sum(p.numel() for p in rlvr_merged_model.parameters()):,}")
    ...
    if args.run_eval:
        ...
        for split in args.splits:
            ...
            split_results = run_full_evaluation(
                rlvr_merged_model, tokenizer, stopping_criteria, 
                eval_dataset, split, 
        ...
        for split in args.splits:
            ...
            generated_outputs = generate_output_batch(
                example_prompts, rlvr_merged_model, tokenizer, stopping_criteria,
                max_new_tokens=args.max_new_tokens
            )
            ...
            for i in range(num_examples):
                ...
                table = [
                    ["RLVR Generated Output", "Ground Truth"],
                    [generated_outputs[i], ground_truth]
                ]
                print(tabulate(table, headers="firstrow", tablefmt="grid"))

                # save output to a text file
                with open(f"rlvr_model_example_output_{split}_ex{i}.txt", "w") as f:
