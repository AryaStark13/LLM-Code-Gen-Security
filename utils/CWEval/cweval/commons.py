# UNCHANGED utils/CWEval/cweval/commons.py
...
# def extract_think_and_code(msg: str) -> tuple[str, str]:
#     """
#     Extract content from <think> and <code> tags.
#     Returns: (think_content, code_content)
#     """
#     import re
    
#     # Extract think content
#     think_pattern = r'<\s*think\s*>(.*?)<\s*/\s*think\s*>'
#     think_matches = re.findall(think_pattern, msg, re.DOTALL | re.IGNORECASE)
#     think_content = think_matches[0].strip() if think_matches else ""
    
#     # Extract code content
#     code_pattern = r'<\s*code\s*>(.*?)<\s*/\s*code\s*>'
#     code_matches = re.findall(code_pattern, msg, re.DOTALL | re.IGNORECASE)
#     code_content = code_matches[0].strip() if code_matches else ""
