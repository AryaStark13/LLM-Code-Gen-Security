...
def store_info_in_header(
    ...
    try:
        ...
        # Map known info types to header keys
        mapping = {
        ...
        key = mapping.get(info_type.strip().lower())
        if key is None:
            # unknown information type; do nothing
            return header

        # Store the content in the header
        header[key] = info_content
        return header
    except Exception:
        # On any unexpected error, return the original header unchanged
        return header
