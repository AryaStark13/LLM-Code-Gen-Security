...
func storeInfoInHeader(infoType string, infoContent string, header map[string]string) map[string]string {
	...
	default:
		// If the information type is not valid, return the original header map
		return header
	}
	return header
}
...
func main() {
	...
	header = storeInfoInHeader("lang", "en", header)
	fmt.Println(header)
	header = storeInfoInHeader("encoding", "gzip", header)
	fmt.Println(header)
	header = storeInfoInHeader("cache", "no-cache", header)
	fmt.Println(header)
	header = storeInfoInHeader("invalid", "invalid", header)
	fmt.Println(header)
}
