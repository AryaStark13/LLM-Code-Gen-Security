...
function hashPasswords(usernames, passwords) {
  ...
  for (let i = 0; i < passwords.length; i++) {
    const salt = crypto.randomBytes(crypto.getHashesSync('sha256').outputLength);
    const hashedPassword = crypto.pbkdf2Sync(passwords[i], salt, 10000, salt.length, 'sha256');
